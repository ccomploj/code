...
* do also with d_count_lead 
	...
	*recode d_count_lead (99=10)
	...
	qui `reg'
	margins `timevar'#`cat'
	marginsplot, `opt_marginsplot'  `xla' `xlarotate' title("Crude Data, by age of first onset, all cohorts")  note("Sample: `samplelabel' | Controls: `ctrls' (none)" "Model: `reg'")  name(g`cat'_`cohort', replace) // by(cohortmin5) // 
	gr export 	"$outpath/fig/main/g_reg_by`timevar'-`cat'_`sample'_`y'.jpg", replace
...
qui `reg'
qui margins `timevar'#`cat' 
marginsplot, `opt_marginsplot'  name(g`cat'_`cohort', replace) `xla' `xlarotate'  // title("`y' over calendar time")  note("Sample: `samplelabel' | Controls: `ctrls' (none)" "Model: `reg'")
gr export 	"$outpath/fig/main/g_reg_by`timevar'-`cat'_`sample'_`y'.jpg", replace
restore	
pause 
*/
