...
loc agethreshold "50" 	// select survey-specific lower age threshold
	...
	***log entire file***
	*log using 	"`h_data'logdoSHARE-2-harmonPart5.txt", text replace name(logDofile) // ends w/ -log close logDofile-
	*log using 	"G:/My Drive/projects/projectMultimorbidity/outfiles/logs/logdoSHARE-2-harmonPart5.txt", text replace name(logDofile) // ends w/ -log close logDofile-
...
****************************************************************************************************
	...
	/***rename key variables (moved to part 4) ***
	rename  panelid ID
	rename  ageyr age 
	rename 	inw7c inw7
...
rename 	cesdr cesd8r
loc 	ptestname 	"cesd8r"
loc 	pthreshold	"3"
gen 	psycher = (`ptestname'>=`pthreshold') if `ptestname'<.  /*validated test as indicator for depression*/

tab	 	depress_selfrep psycher if wave==1
corr 	depress_selfrep psycher if wave==3 /*0.27 in SHARE*/
...
***********************************
do 		"C:/Users/User/Documents/GitHub/code/doFiles/personal/projectMM-part5-subDiseases" 
...
// count 	if hacohort	==1 // 
// count 	if inw1		==1	// 
*keep if hacohort==1 | hacohort==2  /*selected at beginning of file in part 1*/
