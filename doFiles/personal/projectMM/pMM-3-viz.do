...
loc 	opt_marginsplot "ytitle("Mean `ylabel'")  yscale(range(0 .))" // noci  
loc 	ctrls  "`ctrl'"
...
loc 	 reg  	reg `y' `timevar'##`cat' `ctrls' if `sample'==1 & agegrpmin5==`cohort'
 `reg'
margins `timevar'#`cat'
marginsplot, `opt_marginsplot'  title("Crude Data (`data') `cohortlabel'") note("Sample: `samplelabel' | Controls: `ctrls' (none)" "Model: `reg'") name(g`cat'_`cohort', replace)  // by(agegrpmin5)
gr export 	"$outpath/fig/main/g_reg_by`timevar'-`cat'_`sample'_d_count_`cohort'.jpg", replace quality(100)
	...
	*tab age agegrpmin5 if agegrpmin5b>=. // shows that all missing observations are larger than cohort group
	...
	qui `reg'
	qui margins `timevar'#`cat' 
	marginsplot, `opt_marginsplot'  name(g`cat'_`cohort', replace)  title("`ylabel' over calendar time")  note("Sample: `samplelabel' | Controls: `ctrls' (none)" "Model: `reg'")  xla(, ang(20))
...
// 		*tab firstagegrp5,m
	...
	loc cohortlabel: label (`cohortvar') `cohort'
		...
		qui `reg'
		qui margins `timevar'#`cat'
		marginsplot, `opt_marginsplot' `xla' `nolegend' title("Crude Data, by age of first onset (`cohortlabel')") note("Sample: `samplelabel' | Controls: `ctrls' (none)" "Model: `reg'")  name(g`cohort', replace) noci // by(agegrpmin5) //  legend(order(`labellist'))
