...
// 	preserve // 
replace agegrpmin5 = . if agegrpmin5 >= 70 
...
di 		"`timerange'"
replace age = `cohort'+`timerange' if agegrpmin5==`cohort'  & age >`cohort'+`timerange'-5 & age<.
replace timesincefirstobs 	= `timerange'-5 if timesincefirstobs<. & timesincefirstobs>`timerange'-5
replace followup_iwyr 		= `timerange'-5 if followup_iwyr<. & followup_iwyr>`timerange'-5
} 
...
/************************************************
*** +++ Histogram (hist) of main variable +++ *** 
*************************************************
...
preserve 
drop if mi(age) // this is a panel dataset, when plotting a distribution the missing observations are used multiple times, that is not the idea.
loc slabel: variable label `sample'
loc histlist ""
bys ID: egen d_countmax = max(d_count)
la var 		 d_countmax "# diseases (max. before death)"
loc 	y "duration"
foreach y in "d_count" "d_countmax" "age" "ageatfirstobs" "onsetage" {
	di "`slabel'"
hist `y'	if `sample'==1, name(h`y', replace) title(`ylabel') note("`slabel'") width(1) // bin(30)
...
**bar chart with ever had condition within ID **
**# Bookmark #2 could do the same with combinations of diseases
di "$diseaselist"
foreach d of global diseaselist {
loc barlist "`barlist' `d'" 
}
di "`barlist'"

preserve 
collapse (max) `barlist', by(ID)
di "`barlist'"
graph bar `barlist', title("% of IDs who ever (across time) report each disease" "(`data')")
...
loc 	timevar 	"agegrp" /*use age dummies*/
loc 	xla 		"xla(50(5)90)" /*needed for separate sub-plots*/
loc 	opt_marginsplot "ytitle("Mean `ylabel'")  yscale(range(0 .)) noci" // noci  
loc 	ctrls  "`ctrl'"
	...
	*preserve // cannot preserve data twice in stata
	collapse (mean) `y' = `y' 	(count) `y'_freq = `y' if $sample ==1 , by(`cat' `timevar') 
	xtset 	 `cat' `timevar'
...
// qui margins `cat', at(age=(`agethreshold'(10)90)) // this is a lot slower but does the same in this case
marginsplot,  `opt_marginsplot'   title("Crude Data (`data')") note("Sample: `samplelabel'" "Model: `reg'") name(g`cat'_`cohort', replace)  
gr export 	"$outpath/fig/main/g_reg_by`timevar'-`cat'_`sample'_`y'.jpg", replace	quality(100)
]]
*** by covariate levels ***
loc 	 catlist 	"countatfirstobs"
// loc 	 catlist 	"male raeducl countatfirstobs"	

*(a) single combined plot of each cohort group* 
foreach  cat of local catlist {	
loc catlabel: var label `cat'
loc reg  		reg `y' `timevar'##`cat'##agegrpmin5 `ctrls' if $sample == 1 & age<80 // set age limit
qui `reg'
qui margins `timevar'#`cat'#agegrpmin5
marginsplot,  `opt_marginsplot'   by(agegrpmin5) byopts(title("Crude Data, (`data', by `catlabel')")) name(g`cat'_`cohort', replace) // ycommon?
// gr export 	"$outpath/fig/main/g_reg_by`timevar'-`cat'_`sample'_d_count_bycohort.jpg", replace	quality(100)
}

*(b) separate sub-plots per cohort*
foreach  cat of local catlist {	
foreach  cohort of   local levels {
...
loc 	 opt_marginsplot "title("Crude Data `cohortlabel'") ytitle("Mean `ylabel'")" // noci   
loc 	 ctrls  "`ctrl'"
loc 	 reg  	reg `y' `timevar'##`cat' `ctrls' if `sample'==1 & agegrpmin5==`cohort'
qui `reg'
qui margins `timevar'#`cat'
marginsplot, `opt_marginsplot' `xla' title("Crude Data (`data') `cohortlabel'") note("Sample: `samplelabel' | Controls: `ctrls' (none)" "Model: `reg'") name(g`cat'_`cohort', replace)  // by(agegrpmin5)
...
*/
		...
		/*by death age at death categories*
	...
	**# Bookmark #1 in ELSA have no radage apparently, check this
		...
		marginsplot		,   `opt_marginsplot' `xla'  name(g`cat'_`cohort', replace) `xla' title("Crude Data `cohortlabel'") note("Sample: `samplelabel' | Controls: `ctrls' (none)" "Model: `reg'") noci // by(agegrpmin5) 
		gr export 	"$outpath/fig/main/g_reg_by`timevar'-`cat'_`sample'_d_count_`cohort'.jpg", replace quality(100)
		}
// 	restore
	STOP
...
// 		}
	...
	*tab age agegrpmin5 if agegrpmin5b>=. // shows that all missing observations are larger than cohort group
	...
	loc 	cat 		"agegrpmin5b"
	loc 	opt_marginsplot "ytitle("Mean `ylabel'")" // noci  
	loc 	ctrls  		"`ctrl'"
	loc 	reg  		reg `y' `timevar'##`cat' `ctrls' if `sample'==1  
		levelsof hacohort, local(levels)
		loc hacohortlist ""
		loc hacohortvlist ""
		foreach level of local levels {
		loc levellabel: label (hacohort) `level' 
		loc hacohortlist "`hacohortlist' `levellabel' "
		loc hacohortvlist "`hacohortvlist' `level'"
		}
		di "`hacohortlist'"
		di "`hacohortvlist'"
	qui `reg'
	qui margins `timevar'#`cat' 
	marginsplot, `opt_marginsplot'  name(g`cat'_`cohort', replace)  title("`ylabel' over calendar time")  note("Sample: `samplelabel' | Controls: `ctrls' (none)" "Model: `reg'" "cohorts: `hacohortvlist'")  xla(, ang(20))
...
/********************************************
*** +++ count over timesincefirstobs +++ *** 
********************************************
rename 	timesincefirstobs timesincefirstobs2
gen 	timesincefirstobs = followup_iwyr
// 		keep if cohortselection==1
loc 	sample 		"$sample"
// 	loc sample "sfull5"
loc samplelabel:	var label `sample'
loc 	y 			"d_count"
loc 	ylabel: 	var label `y'
loc 	cat  	  	"agegrpmin5" 
loc 	timevar 	"timesincefirstobs" // timesincefirstobs followup_iwyr
// 	loc 	xla 		""
// loc 	xlarotate	""	
loc 	opt_marginsplot "ytitle("Mean `ylabel'") " // noci yscale(range(0 .))  
loc 	ctrls  		"`ctrl'"
...
**************************************************
*** +++ count over age by first onset age  +++ *** 
**************************************************
...
// 	set graphics on
		...
		loc 	y 			"d_count"
		loc 	ylabel: 	var label `y'
		loc 	cat  	  	"onsetagegrp5" 
		loc 	timevar 	"agegrp" 
		loc 	opt_marginsplot "ytitle("Mean `ylabel'")" // noci  
		loc 	ctrls  		"`ctrl'"
		loc 	xla 		"xla(50(5) 90)" /*needed for separate sub-plots (to make entry age visible)*/
	loc cohortvar 	"agegrpmin5"
	loc cohort 		"50"
// 		loc nolegend "legend(off)"
	...
	loc cohortlabel: label (`cohortvar') `cohort'
		...
		qui `reg'
		qui margins `timevar'#`cat'
		marginsplot, `opt_marginsplot' `xla' yla(0 (1) 4) `nolegend' title("Crude Data, by onset age" "(`cohortlabel')" "`data'") note("Sample: `samplelabel' | Controls: `ctrls' (none)" "Model: `reg'")  name(g`cohort', replace) noci // by(agegrpmin5) //  legend(order(`labellist'))
		gr export 	"$outpath/fig/main/g_byonsetage_`cohort'.jpg", replace quality(100)
// 		loc nolegend "legend(off)"
	}
// 		gr combine g50 g55 g60 g65, ycommon 
		// grc1leg2 g50 g55 g60 g65, scheme(s1color) legendfrom(g55) ycommon  // allows for single legend in combined graph
**# Bookmark #1cannot use same legend, need to make sure colors are correct
		STOP
		*/
...
loc cohortlabel :  label (agegrpmin5) `v'
sts graph if agegrpmin5==`v', survival by(c) legend(pos(6) rows(3)) name(g`v') title("Crude Data `cohortlabel'")	
gr export 	"$outpath/fig/main/g_surv_`sample'_bycountatfirstobs_`v'.jpg", replace quality(100)
}
restore 
STOP 
*/
	...
	** change label of resulting densities **
	...
	// loc 	onsetd_`y'_uncenslabel: var label onsetd_`y'_uncens // removed this var
	...
	*/
		...
		** generate higher order onset (self-reported) ** 
		...
		gen second_age_`d' = radiag`d' if radiag`d' > onsetage_g2  // define the 2nd onset (using radiag)
	}
		egen onsetage_g2_2th = rowmin(second_age_*) // youngest age among all first onset
		*bro ID wave onsetage_g2_2th radiag*
		sum second_age_* onsetage_g2_2th 
		foreach d of global diseasecodelist {
		gen third_age_`d' = radiag`d' if radiag`d' > onsetage_g2_2th 
	}
		egen onsetage_g2_3th = rowmin(third_age_*)
		sum third_* onsetage_g2_3th
		drop second_* third_* 
		// now define if the disease first emerged at the same time as the 2nd (3rd) disease (for each disease a variable) (for each number, multiple diseases are possible if they emerge at the same time)
		foreach d of global diseasecodelist {
		gen radiag2th`d' = radiag`d'  if radiag`d'  == onsetage_g2_2th // if that specific disease has same age as 2nd onset
		gen radiag3th`d' = radiag`d'  if radiag`d'  == onsetage_g2_3th 
		}
		sum radiag2th* radiag3th* 
		...
		** generate higher order onset (observed) **
		...
		foreach d of global diseasecodelist {
		gen second_onset_age_`d' = onsetd_`d' if onsetd_`d' > onsetage 
		}
		egen onsetage_2th = rowmin(second_onset_age_*) // youngest age among all first onset
		*bro ID wave onsetage onsetd_* second_onset_age*

		foreach d of global diseasecodelist {
		gen third_onset_age_`d' = onsetd_`d' if onsetd_`d' > onsetage_2th 
		}
		...
		foreach d of global diseasecodelist {
		gen onset2thd_`d' = onsetd_`d'  if onsetd_`d'  == onsetage_2th // if that specific disease has same age as 2nd onset
		gen onset3thd_`d' = onsetd_`d'  if onsetd_`d'  == onsetage_3th 
		}
		sort ID time
		*bro ID wave age onsetage onsetage_2th onset2th* 
		
		
	**# Bookmark #1 should actually only have one observation per individual when calculating densities
			preserve 
			bysort ID (time): keep if _n == 1
		loc i "3" // 2 3 
		loc sample 		"sfull5healthy"  // do only for healthy sample
		loc samplelabel: variable label `sample'
		foreach y of local templist { // these are the disease codes, defined at beginning of this block
	...
	//  	gr export 	"$outpath/fig/main/g_kden_`sample'_radiagonset`i'thalld.pdf", replace 
			**for observed**
		di 			"`kdenlist_obs'"
		twoway 		`kdenlist_obs', title("density: `i'-th onset by disease (observed)"  "`samplelabel'" "`data'") name(k2, replace)
		...
		** conditional on the first disease being CV, what is 2nd?** (CV=heart, strok) (cannot run with code above due to repeat names (more error prone))
		gen firstonset_cv = (onsetage == onsetd_heart) | (onsetage == onsetd_strok)
		*bro ID wave age onsetage onsetd_* firstonset_cv
	//	foreach d of global diseasecodelist { // incl. all diseases
			local list "hibp diab lung depr osteo cancr arthr demen"
		foreach d of local list { // excluding Cardiovascular	
		gen second_onset_age_`d' = onsetd_`d' if onsetd_`d' > onsetage   & firstonset_cv==1 // what is second onset, if first onset is cardiovascular
		}		
		egen onsetage_2th = rowmin(second_onset_age_*) // youngest age among all first onset
	...
	// 	foreach d of global diseasecodelist {
		...
		*bro ID wave age onsetage onsetage_2th onset2th* 
		...
		loc samplelabel: variable label `sample'
		*foreach y of local templist { // these are the disease codes, defined at beginning of this block
		foreach y of local list { // excluding Cardiovascular	
		kdensity  onset`i'thd_`y'       if `sample' == 1, gen(point`y'_obs density`y'_obs)  nograph  // bw(2)
		loc 	  kdenlist_obs 	"`kdenlist_obs' (line density`y'_obs point`y'_obs)"
		loc 	onset`i'thd_`y'label: var label onsetd_`y'
		la var 	density`y'_obs "`onsetd_`y'label'"
		}
			**for observed**
		di 			"`kdenlist_obs'"
		twoway 		`kdenlist_obs', title("density: `i'-th onset by disease (observed)" "conditional on first disease being CV" "`samplelabel'" "`data'") name(k2, replace)
		gr export 	"$outpath/fig/main/g_kden_`sample'_`i'thonsetalld_firstiscv.pdf", replace 
			restore
		STOP	
		*/
